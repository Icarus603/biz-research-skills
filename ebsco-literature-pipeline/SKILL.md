...
```
Topic
  → Phase 1: bibliography_agent  (S2 + OpenAlex + Crossref + WebSearch → verified paper list)
...
## Phase 1: Bibliography Agent

Read and execute `agents/bibliography_agent.md` as the Bibliography Agent.
...
Apply the corresponding filter from `references/journal_lists.md` to both the API queries (venue filter) and the EBSCO queries (SO filter). If user says "all journals", no filter.

**What it does**:
- Systematic search via Semantic Scholar API, OpenAlex API, Crossref API, and WebSearch
- Deduplicates via Semantic Scholar ID (same paper from multiple databases = one entry)
- Existence verification is built-in: three-index triangulation (S2 + OpenAlex + Crossref). Papers unmatched across all three → flagged `not_found`, still reported but not attempted for download.
- API protocols: `references/semantic_scholar_api_protocol.md`, `references/openalex_api_protocol.md`, `references/crossref_api_protocol.md`

**Output** — structured paper list, one record per paper:
```
{
  idx: int,           # 1-based sequential
  title: string,
  first_author: string,
  year: int,
  venue: string,
  doi: string | null,
  oa_url: string | null,   # open access PDF URL if available
  s2_id: string | null,
  not_found: bool          # true if unmatched across all three indexes
}
```

Show user the list (title + year + venue, N papers). Ask: "Found N papers. Proceed with download?"
...
```
Discovery:   N papers  (bibliography_agent via S2 + OpenAlex + Crossref + WebSearch)
...
| File | Purpose |
|------|---------|
| `agents/bibliography_agent.md` | Full bibliography agent — systematic search + dedup + existence check |
